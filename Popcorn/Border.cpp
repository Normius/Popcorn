...
void CBorder::Draw(HDC hdc, RECT& paintArea, HPEN bgBlackPen, HBRUSH bgBlackBrush)
{
    //Draw left level border
    for (int i = 0; i < 50; ++i)
    {
        DrawVerticalElement(hdc, 2, 1 + i * 4, bgBlackPen, bgBlackBrush);
    }

    //Draw right level border
    for (int i = 0; i < 50; ++i)
    {
        DrawVerticalElement(hdc, 201, 1 + i * 4, bgBlackPen, bgBlackBrush);
    }

    //Draw top level border
    for (int i = 0; i < 50; ++i)
    {
        DrawHorizontalElement(hdc, 3 + i * 4, 0, bgBlackPen, bgBlackBrush);
    }
}

//Draw vertical element (tile) of level border 
void CBorder::DrawVerticalElement(HDC hdc, int x, int y, HPEN bgBlackPen, HBRUSH bgBlackBrush)
{
    //Blue border line
...
void CBorder::DrawHorizontalElement(HDC hdc, int x, int y, HPEN bgBlackPen, HBRUSH bgBlackBrush)
{
    SelectObject(hdc, borderBluePen);
    SelectObject(hdc, borderBlueBrush);
